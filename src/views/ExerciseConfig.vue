<template>
    <div class="exercise-plan-management">
      ...
      <div class="search-bar">
        ...
        <el-button @click="searchExercisePlan" type="primary" icon="el-icon-search">搜索</el-button>
        <el-button @click="resetFilters" type="warning">重置</el-button>
      </div>
  
      <!-- 新增运动计划按钮 -->
      <el-button @click="addExercisePlan" type="primary" class="add-button">新增运动计划</el-button>
      ...
      <el-table :data="currentPageExercisePlans" border>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" @click="editExercisePlan(scope.row)">编辑</el-button>
            <el-button
              size="mini"
      ...
      <!-- 编辑弹窗 -->
      <el-dialog :visible.sync="isEditDialogVisible" title="设置运动计划" width="40%">
  ...
  export default {
    data() {
      return {
        // 运动计划数据
        exercisePlans: [
          { title: '力量训练计划1', summary: '描述1', category: '力量训练', content: '内容1', status: 1 },
          { title: '有氧训练计划2', summary: '描述2', category: '有氧训练', content: '内容2', status: 1 },
          { title: '灵活性训练计划3', summary: '描述3', category: '灵活性训练', content: '内容3', status: 0 },
          // 可以继续添加更多数据
        ],
        searchQuery: '',
        categoryFilter: '',
    ...
    methods: {
      ...
      updateCurrentPageExercisePlans() {
        ...
        this.currentPageExercisePlans = this.filteredExercisePlans.slice(startIndex, startIndex + this.pageSize);
      },
      ...
      // 保存编辑的运动计划信息
      saveExercisePlan() {
        const index = this.exercisePlans.findIndex(plan => plan.title === this.currentExercisePlan.title);
        if (index !== -1) {
      ...
      // 新增运动计划
      addExercisePlan() {
        this.currentExercisePlan = { title: '', summary: '', category: '', content: '', status: 1 }; // 清空表单
        this.isEditDialogVisible = true; // 打开编辑弹窗
      }
    }
  };
